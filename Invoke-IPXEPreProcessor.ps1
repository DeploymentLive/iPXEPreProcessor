    ...
    elseif ( $line -match '^\s*\#?(INCLUDE|include)\b\s*(?<Path>[^\#]*)\s*(\#.*)?$' ) {
        ...
        elseif ( test-path ( $Matches.path ) ) {
            ...
            foreach ( $inc in $include ) {
                if ( test-path ( join-path $inc $Matches.path ) ) {
                    # Relative to the parent file.
                    $found = (join-path $inc $Matches.path)
                }                
            }
        }
...
"@ | write-output
    ...
    elseif ( $line -match '^\s*\b(?<op>(break|continue))\b\s*(\#.*)?$' ) {
        ...
        $currentCommand = $commandStack.ToArray() | ? { $_.op -match 'while' } | select-object -first 1
        if ( $CurrentCommand.op -notmatch '(while)' ) { throw "end while is not matching to an WHILE command" }
        $label = $CurrentCommand.label
@"
# $($Line)
...
"@ | write-output
    ...
    elseif ( $line -match '^\s*\#?(?<op>sub)\b\s*(?<name>[a-zA-Z0-9_-]*)\s*(\#.*)?$' ) {
...
"@ | write-output
    ...
    elseif ( $line -match '^\s*\#?(?<op>call)\b\s*(?<name>\$?\{?[a-zA-Z0-9_-]*\}?)\s*(?<arg>.*)$' ) {
...
goto sub_$($matches.name.tolower()) || prompt failed to call sub_$($matches.name.tolower()) 
:$($label)_returnpoint
...
prompt THROW Failed to return to $($label)
"@ | write-output
